...
namespace TransferPayment\Model\Map;
 ...
 *
 */
class TransferPaymentConfigTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'TransferPayment.Model.Map.TransferPaymentConfigTableMap';
     ...
     */
    const TABLE_NAME = 'transfer_payment_config';

    /**
     * The related Propel class for this table
     */
    const OM_CLASS = '\\TransferPayment\\Model\\TransferPaymentConfig';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'TransferPayment.Model.TransferPaymentConfig';
     ...
     */
    const NAME = 'transfer_payment_config.NAME';
    ...
    {
        // attributes
        $this->setName('transfer_payment_config');
        $this->setPhpName('TransferPaymentConfig');
        $this->setClassName('\\TransferPayment\\Model\\TransferPaymentConfig');
        $this->setPackage('TransferPayment.Model');
    ...
    {
        return $withPrefix ? TransferPaymentConfigTableMap::CLASS_DEFAULT : TransferPaymentConfigTableMap::OM_CLASS;
    }
    ...
    {
        $key = TransferPaymentConfigTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = TransferPaymentConfigTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + TransferPaymentConfigTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = TransferPaymentConfigTableMap::OM_CLASS;
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            TransferPaymentConfigTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = TransferPaymentConfigTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = TransferPaymentConfigTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(TransferPaymentConfigTableMap::DATABASE_NAME)->getTable(TransferPaymentConfigTableMap::TABLE_NAME);
    }
    ...
    {
      $dbMap = Propel::getServiceContainer()->getDatabaseMap(TransferPaymentConfigTableMap::DATABASE_NAME);
      if (!$dbMap->hasTable(TransferPaymentConfigTableMap::TABLE_NAME)) {
        $dbMap->addTableObject(new TransferPaymentConfigTableMap());
      }
    }
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \TransferPayment\Model\TransferPaymentConfig) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(TransferPaymentConfigTableMap::DATABASE_NAME);
            $criteria->add(TransferPaymentConfigTableMap::NAME, (array) $values, Criteria::IN);
        }

        $query = TransferPaymentConfigQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) { TransferPaymentConfigTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) { TransferPaymentConfigTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return TransferPaymentConfigQuery::create()->doDeleteAll($con);
    }
...
} // TransferPaymentConfigTableMap
// This is the static code needed to register the TableMap for this table with the main Propel class.
//
TransferPaymentConfigTableMap::buildTableMap();
